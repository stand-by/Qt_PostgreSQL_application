...
CREATE VIEW short_move_goods AS
 SELECT document.id_document,
  ...
  WHERE (((type_of_document.name_of_type)::text = 'Переміщення з складу в магазин'::text) OR ((type_of_document.name_of_type)::text = 'Переміщення з магазину в склад'::text))
...
CREATE VIEW short_purchase_goods AS
 SELECT document.id_document,
  ...
  WHERE ((type_of_document.name_of_type)::text = 'Закупка на склад'::text)
  ORDER BY document.id_document;
...
CREATE VIEW short_sell_goods AS
 SELECT document.id_document,
  ...
  WHERE ((type_of_document.name_of_type)::text = 'Продаж із магазину'::text)
  ORDER BY document.id_document;
...
SELECT pg_catalog.setval('contractor_id_contractor_seq', 8, true);
...
Склад підприємства	1	442363987	просп. Перемоги, 37	1
Магазин підприємства	2	442048193	просп. Перемоги, 37А	2
\.
...
SELECT pg_catalog.setval('document_id_document_seq', 23, true);
...
SELECT pg_catalog.setval('document_on_the_move_of_goods_id_document_on_the_move_of_goods_', 31, true);
...
4	Двигун Honda	3
10	Двигун Acura	3
11	Масло для Mitsubishi	11
5	BBS	6
6	Pioneer	8
7	Двірники задні	9
8	Двірники передні	9
9	Капот для Nissan	10
\.
...
SELECT pg_catalog.setval('goods_id_goods_seq', 11, true);
...
КийАвто	12345	957894545	м. Київ, вул. Перемоги, 21	3
Winner Automotive	789	445674563	м. Київ, вул. Перемоги 23	5
Nissan	1234567899	1223345566	Корабельна, 4	6
Тов Honda suppliers	4594564	444231548	вул. Перемоги, 34	7
Mitsubishi Motors	564865456	445879687	вул. Перемоги, 28	8
\.
...
Петренко	Денис	Анатолiйович	1996-01-01	м.Київ, вул. Перемоги, 22	123	957217898	4
\.
...
1	Закупка на склад
2	Продаж із магазину
3	Переміщення з складу в магазин
4	Переміщення з магазину в склад
\.
...
SELECT pg_catalog.setval('type_of_goods_id_type_of_goods_seq', 11, true);
...
GRANT ALL ON TABLE short_move_goods TO manager;
GRANT SELECT ON TABLE short_move_goods TO inspector;
GRANT SELECT ON TABLE short_move_goods TO vendor;
...
GRANT ALL ON TABLE short_purchase_goods TO manager;
GRANT SELECT ON TABLE short_purchase_goods TO inspector;
GRANT SELECT ON TABLE short_purchase_goods TO vendor;
...
GRANT ALL ON TABLE short_sell_goods TO manager;
GRANT SELECT ON TABLE short_sell_goods TO inspector;
GRANT SELECT ON TABLE short_sell_goods TO vendor;
